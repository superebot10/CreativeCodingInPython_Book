...
# checks if character hits bad candy, schedule end_game_over
...
def check_hits():
    ...
    for candy in candy_list:
        if collision(mychar, candy, 30):
            ...
            # find where in list and remove a nd update score
...
# Function handles when user stops pressing arrow key
def end_input(event):
    global move_direction
    move_direction = "None"
...
canvas.bind_all('<KeyPress>', check_input) # Bind key input
